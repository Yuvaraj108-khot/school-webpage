...
router.post('/send-otp', async (req, res) => {
    ...
    const mailOptions = {
                ...
                <div style="background-color: #ffffff; border-radius: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.05); overflow: hidden; border: 1px solid #eef2f5;">
                    ...
                    <div style="background: linear-gradient(135deg, #1a3a5c 0%, #2c4a6e 100%); padding: 30px 20px; text-align: center; color: #ffffff;">
                        <div style="margin-bottom: 12px;">
                            <img src="cid:school_logo" alt="Logo" style="width: 70px; height: 70px; border-radius: 50%; border: 3px solid rgba(255, 255, 255, 0.2); object-fit: cover; display: inline-block;">
                        </div>
                    ...
                    <div style="padding: 35px 25px; color: #333333;">
                        ...
                        <p style="font-size: 14px; line-height: 20px; color: #555555; margin: 0 0 20px;">
                            We received a request to reset your student/parent portal password. Use the verification code below to complete your reset. This code will expire in <strong style="color: #e74c3c;">10 minutes</strong>.
                        </p>
        ...
        attachments: [{
            ...
            path: path.join(__dirname, '../../images/logo_small.png'),
            cid: 'school_logo'
        }]
    };
    ...
    try {
        ...
        res.status(500).json({ error: 'Failed to send email. Check server email configuration in .env.' });
    }
});
...
router.post('/verify-otp', (req, res) => {
    ...
    verifiedResetStore.set(emailLower, Date.now() + 10 * 60 * 1000);
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const teacher = await prisma.teacher.findFirst({
            where: {
                OR: [
                    { staff_id: { equals: username.trim() } },
                    { email: { equals: userLower } }
                ]
            }
        });
        ...
        const student = await prisma.student.findFirst({
            where: {
                OR: [
                    { student_code: { equals: username.trim() } },
                    { email: { equals: userLower } },
